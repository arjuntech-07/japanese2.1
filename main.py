...
from manga_ocr_utils import get_text_from_image  # ← Japanese (keep this)
...
INPUT_FOLDER = "/kaggle/input/test1-0/test1"
CROP_DIR = "crops1"
OUTPUT_FOLDER = "/kaggle/working/translated_manga"
MODEL_PATH = "/kaggle/input/yolov8/pytorch/default/1/best.pt"


# ─────────────────────────────────────────────
# PHASE 2: OCR Processing
# ─────────────────────────────────────────────
def run_ocr_batch(crop_paths):
    """
    Takes a list of crop image paths,
    Runs manga_ocr on each,
    Returns {"filename": "text"}
    """
    print(f"\n📖 Phase 2: Running OCR on {len(crop_paths)} bubbles...")
    ...
    for i, img_path in enumerate(crop_paths):
        ...
        try:
            ...
            text = get_text_from_image(pil_image)

            if text:
                text = text.strip()

            results[filename] = text if text else ""

            if (i + 1) % 10 == 0:
                print(f"  Processed {i + 1}/{len(crop_paths)} bubbles...")

        except Exception as e:
            print(f"  ⚠️ Error processing {filename}: {e}")
            results[filename] = ""

    print(f"✅ Phase 2 Done. OCR Complete.")
    return results
...
def main():
    ...
    # 1. Setup Folders
    for folder in [INPUT_FOLDER, CROP_DIR, OUTPUT_FOLDER]:
    ...
    ocr_results = run_ocr_batch(all_crops)
