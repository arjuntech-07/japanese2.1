...
def process_page_assembly(original_image_path, bubble_data_list, ocr_results):
    ...
    for bubble in bubble_data_list:
        ...
        if english and english.strip():
            # Clean up reasoning tags from Qwen/DeepSeek safely
            if "</think>" in english:
                parts = english.split("</think>")
                text_after_tag = parts[-1].strip()

                if text_after_tag:
                    # Normal case: the translation is after the tag
                    english = text_after_tag
                else:
                    # Edge case: the AI accidentally put the translation INSIDE the tags
                    english = parts[0].replace("<think>", "").replace("<think>\n", "").strip()
    ...
    for bubble in bubble_data_list:
        ...
        # FIX: Only attempt to render text if there is actually text to draw
...
def main():
    ...
    # FIX: Use sorted() to ensure pages process in alphabetical/numerical order (01, 02, 03...)
